...
int main()
{
    ...
    double z=0.04,s,s1,z1,B,f,max=0,mx,max1=0,s2;
    ...
    for(i=0;i<len;i++)
       {
           ...
           if(a[i]>B)
             LR[i]=log(pow(a[i]/B,a[i])*pow((len-a[i])/(len-B),len-a[i]));
           else
             LR[i]=0;
		 v1.push_back(make_pair(max1,v[i]));
       }
    for(i=0;i<len;i++)
       {
    ...
    for(i=0;i<len*1.01;i++)
       {
           for(j=0;j<len*1.01;j++)
                if(sqrt(pow(v2[i].first-v2[j].first,2)+pow(v2[i].second-v2[j].second,2))<=z)
                  b[i]++;
           if(b[i]>B)
             LR2[i]=log(pow(b[i]/B,b[i])*pow((len*1.01-b[i])/(len*1.01-B),len*1.01-b[i]));
           else
             LR2[i]=0;
       }
   ...
   for(i=0;i<kl;i++)
    ...
    for(i=0;i<kl;i++)
       //if(float(pos[i])/float((len*.01+1.0))<0.01)
         myfile<<v5[hspt[i]].second.second.first<<' '<<v5[hspt[i]].second.second.second<<" "<<v5[hspt[i]].second.first<<'\n';
    myfile.close();
    return 0;
}
